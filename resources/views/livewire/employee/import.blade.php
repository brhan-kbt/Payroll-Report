            ...
            <div
                    ...
                    <div class="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-md">
                        <div class="grid grid-cols-3 gap-4 text-sm">
                            <div class="text-center">
                                ...
                                <div class="text-2xl font-bold text-blue-700 dark:text-blue-300">{{ $importStats['total'] }}</div>
                            </div>
                            <div class="text-center">
                                <div class="font-semibold text-green-600 dark:text-green-400">Success</div>
                                <div class="text-2xl font-bold text-green-700 dark:text-green-300">{{ $importStats['success'] }}</div>
                            </div>
                            <div class="text-center">
                                <div class="font-semibold text-red-600 dark:text-red-400">Errors</div>
                                <div class="text-2xl font-bold text-red-700 dark:text-red-300">{{ $importStats['errors'] }}</div>
                            </div>
                        </div>
                    </div>
                @endif
                        ...
                        <div class="max-h-32 overflow-y-auto border border-red-200 dark:border-red-800 rounded-md">
                            <table class="min-w-full divide-y divide-red-200 dark:divide-red-800">
                                <thead class="bg-red-50 dark:bg-red-900/20">
                                    <tr>
                                        <th class="px-3 py-2 text-left text-xs font-medium text-red-600 dark:text-red-400">Row</th>
                                        <th class="px-3 py-2 text-left text-xs font-medium text-red-600 dark:text-red-400">Employee ID</th>
                                        <th class="px-3 py-2 text-left text-xs font-medium text-red-600 dark:text-red-400">Error</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-red-100 dark:divide-red-900/30">
                                    @foreach ($importErrors as $error)
                                        <tr class="bg-white dark:bg-gray-800 hover:bg-red-50 dark:hover:bg-red-900/10">
                                            <td class="px-3 py-2 text-sm text-gray-900 dark:text-white">{{ $error['row'] }}</td>
                                            <td class="px-3 py-2 text-sm text-gray-900 dark:text-white">{{ $error['employee_id'] }}</td>
                ...
                <div class="mt-6 flex justify-end space-x-2">
                    ...
                    <button type="button" wire:click="import" @if (!$file) disabled @endif
                        ...
                        <span class="flex items-center transition-opacity duration-200" wire:loading.class="opacity-0" wire:loading.remove.class="opacity-100">
                            <i class="fas fa-file-import mr-2"></i> Import
                        </span>

                        <!-- Loading spinner that only shows during import -->
                        <span class="flex items-center absolute inset-0 justify-center transition-opacity duration-200 opacity-0 " wire:loading.class="opacity-100">
